...
const createInjectableJsFile = function (filename, injectJs) {
  ...
  let tempFilePath = path.resolve(app.getPath('temp') + 'lintbot-' + classify(filename) + '.js');

  inject += `\n${injectJs}`;

  fs.writeFileSync(tempFilePath, inject);

  lintbotMain.debug(`@@${tempFilePath}@@`);
...
const getNewBrowserWindow = function (filename, userOpts, injectJs) {
  ...
  return new BrowserWindow({
    x: (lintbotMain.isDebug()) ? 100 : 5000,
    y: (lintbotMain.isDebug()) ? 100 : 5000,
    center: false,
    width: opts.width,
    height: opts.height,
    show: (lintbotMain.isDebug()) ? true : false,
    frame: (lintbotMain.isDebug()) ? true : false,
...
const getWindowLoadingOptions = function (listenerId) {
  return {
    httpReferrer: `https://${listenerId}.running-task-windows.lintbot.web-dev.tools/`,
...
const load = function (listenerId, url, opts, injectJs, next) {
  ...
  const cleanup = function () {
    ipcRenderer.removeAllListeners('__lintbot-hidden-browser-devtools-loaded');
    ipcRenderer.removeAllListeners('__lintbot-hidden-browser-window-loaded');
    ipcRenderer.removeAllListeners('__lintbot-hidden-browser-window-fonts-loaded');
    ipcRenderer.removeAllListeners('__lintbot-hidden-browser-har-generation-succeeded');
    win.closeDevTools();
  };
  ...
  ipcRenderer.on('__lintbot-hidden-browser-devtools-loaded', function (e) {
    process.nextTick(function () {
      win.loadURL(getUrl(url), getWindowLoadingOptions(listenerId));
      win.webContents.executeJavaScript(`window.__lintbotHiddenTestingWindowId = ${listenerId};`);
    });
  });

  ipcRenderer.on('__lintbot-hidden-browser-window-fonts-loaded', function (e, details) {
  ...
  ipcRenderer.on('__lintbot-hidden-browser-window-loaded', function (e, details) {
    if (details.location != PRELOAD_PATH) {
      windowLoaded = true;
      checkForFinalLoad(details.location);
    }
  ...
  ipcRenderer.once('__lintbot-hidden-browser-har-generation-succeeded', function (e, har) {
    cleanup();
    next(win, har);
  });
  ...
  if (speed) {
    lintbotMain.debug(`Network: ${networkName}, ${speed.latency}ms⬌, ${speed.downloadThroughput}kB/s⬇, ${speed.uploadThroughput}kB/s⬆`)
