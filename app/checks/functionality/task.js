(function () {
  ...
  const check = function (file, next) {
    ...
    const cleanup = function () {
      ...
      ipcRenderer.removeAllListeners('__lintbot-functionality-test-pass-' + listenerLabel);
      ipcRenderer.removeAllListeners('__lintbot-functionality-test-fail-' + listenerLabel);
      ipcRenderer.removeAllListeners('__lintbot-functionality-test-debug-' + listenerLabel);
    ...
    if (!fileExists.check(pagePath)) {
      lintbotMain.send('check-group:item-complete', group, listenerLabel, displayLabel, [`The file \`${file.path}\` is missing or misspelled`]);
      return next();
    }
    ...
    ipcRenderer.on('__lintbot-functionality-error', function (event, message, line, filename) {
      ...
      if (message && !filename && !line) lintbotMain.send('check-group:item-complete', group, listenerLabel, displayLabel, [`${message}`]);
      if (message && filename && !line) lintbotMain.send('check-group:item-complete', group, listenerLabel, displayLabel, [`${message} — \`${filename}\``]);
      if (message && filename && line) lintbotMain.send('check-group:item-complete', group, listenerLabel, displayLabel, [`${message} — \`${filename}\` on line ${line}`]);

      next();
    });

    ipcRenderer.on('__lintbot-functionality-test-pass-' + listenerLabel, function(event) {
    ...
    ipcRenderer.on('__lintbot-functionality-test-fail-' + listenerLabel, function(event, reason) {
      cleanup();
      lintbotMain.send('check-group:item-complete', group, listenerLabel, displayLabel, [`The website isn’t functioning as expected: ${reason}`]);
      next();
    });

    ipcRenderer.on('__lintbot-functionality-test-debug-' + listenerLabel, function (event, ...e) {
      lintbotMain.debug(...e);
    });
  ...
  filesToCheck.forEach((file, i) => {
    ...
    lintbotMain.send('check-group:item-new', group, filesToCheck[i].listenerLabel, filesToCheck[i].displayLabel);
  });
