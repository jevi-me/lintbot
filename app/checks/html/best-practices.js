...
const check = function (checkGroup, checkId, checkLabel, fileContents, lines, next) {
  ...
  if (missingDocumentTags && missingDocumentTags.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, missingDocumentTags, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (forceLineBreaks && forceLineBreaks.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, forceLineBreaks, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkClosingPSameLine && checkClosingPSameLine.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkClosingPSameLine, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkMissingOptionalTags && checkMissingOptionalTags.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkMissingOptionalTags, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkCodeStyle && checkCodeStyle.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkCodeStyle, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkEmptyLines && checkEmptyLines.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkEmptyLines, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkDoubleSpacing && checkDoubleSpacing.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkDoubleSpacing, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkSpaceBeforeCloseGT && checkSpaceBeforeCloseGT.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkSpaceBeforeCloseGT, false, false, ERROR_MESSAGE_STATUS.SKIP);
    return next();
  }
  ...
  if (checkIndentation && checkIndentation.length > 0) {
    lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel, checkIndentation);
    return next();
  }

  lintbotMain.send('check-group:item-complete', checkGroup, checkId, checkLabel);
  next();
};
